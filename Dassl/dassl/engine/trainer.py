...
class TrainerBase:
    ...
    All subclass should override
    ``forward_backward`` and ``model_inference``.
    """
    ...
    def __init__(self, cfg):
        ...
        self._param_freeze_status = OrderedDict()
    ...
    def train(self, start_epoch, max_epoch, idx=-1,global_epoch=-1,is_fed=False,global_weight=None, fedprox=False, mu=0.5, debug_mode=False):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            # Skip local epoch testing in debug mode
            if not debug_mode:
                self.after_epoch()
    ...
    def after_train(self,idx=-1,epoch=0,is_fed=False):
        ...
        if debug_mode:
            is_final_global_round = (epoch + 1 == self.cfg.OPTIM.ROUND)
            if not is_final_global_round:
                do_test = False
                print(f"Debug mode: skip testing for client {idx} at round {epoch+1}/{self.cfg.OPTIM.ROUND}")
            else:
                print(f"Debug mode: run testing for client {idx} at final round {epoch+1}/{self.cfg.OPTIM.ROUND}")
...
class SimpleTrainer(TrainerBase):
    ...
    def train(self,idx=-1,global_epoch=0,is_fed=False,global_weight=None, fedprox=False, mu=0.5, debug_mode=False):
        super().train(self.start_epoch, self.max_epoch,idx,global_epoch,is_fed,global_weight,fedprox,mu,debug_mode)
    ...
    def test(self, split=None, is_global=False, current_epoch=0, idx=-1, global_test=False):
        ...
        if split == "val" and hasattr(self, 'val_loader') and self.val_loader is not None:
            ...
            # Determine data loader based on test mode
            if global_test and not self.is_special_dataset:
                # Global test mode
                ...
                if hasattr(self, 'test_loader') and self.test_loader is not None:
                    ...
                    # Fallback to first federated test set
                ...
                if data_loader is None:
                    print(f"Warning: client {idx} has no dedicated test set, falling back to global test set")
                    if hasattr(self, 'test_loader') and self.test_loader is not None:
                        data_loader = self.test_loader
                    else:
                        raise ValueError(f"Client {idx} has no available test data and no global test set")
            else:
                # Standard test mode
        ...
        # Log test type
        test_type = "global" if global_test and not self.is_special_dataset else f"client {idx}"
        ...
        for k, v in results.items():
            ...
            if not is_global and idx >= 0:
                tag = f"{tag}/{str(idx)}"
            elif global_test and not self.is_special_dataset:
                tag = f"{tag}/global"
    ...
    def model_inference(self, input):
        ...
        # Return first element if output is tuple (typically logits)
