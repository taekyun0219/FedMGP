...
class OxfordPets(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        # train_n, val_n, test_n = self.subsample_classes(total_train, val, test, subsample='new')
        train, val, test = self.subsample_classes(total_train, val, test, subsample=subsample)
        ...
        if cfg.DATASET.USERS > 0 and cfg.DATASET.SUBSAMPLE_CLASSES != 'all':
            if cfg.DATASET.USEALL:
                federated_train_x = defaultdict(list)
                for idx in range(cfg.DATASET.USERS):
                    federated_train_x[idx] = test
